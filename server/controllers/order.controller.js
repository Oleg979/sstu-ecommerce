...
router.get("/accept/:id", verifyAdmin, (req, res) => {
  Order.findById(req.params.id, (err, order) => {
    order.status = "Accepted";
    order.acceptorId = req.userId;
    order.save();
    if (err)
      return res
        .status(500)
    ...
    User.findById(order.customerId, (err, user) => {
      sendMail(
        ...
        `<!doctype html>
                                      ...
                                      <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                        <tr>
                                          <td>
                                            ...
                                            <h2>Мы рады сообщить, что Ваш заказ <b>${order._id}</b> одобрен. Сумма к оплате: </h2>
                                                ...
                                                <tr>
                                                  <td align="center">
                                                    <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                                      <tbody>
                                                        <tr>
                                                          <td rowspan="2"> <a><span style="font-size:30px;">${order.sum}₽</span></a> </td>
                                                        </tr>
                                                      </tbody>
                                                    </table>
                                                  </td>
                    ...
                    `,
        `${user.name}, Ваш заказ принят`
      );
      res.status(200).send({ res: true, order });
    });
  });
});
